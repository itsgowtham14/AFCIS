...
exports.getSectionAnalytics = async (req, res) => {
  try {
    ...
    // Get all responses - filter by section using studentSection field
    const allResponses = await FeedbackResponse.find({ formId: { $in: formIds } })
      .populate('studentId', 'universityId personalInfo');
    
    // Filter responses to only include those from this specific section
    const responses = allResponses.filter(r => {
      ...
      if (r.studentSection) {
        ...
        const normalizedTargetSection = String(sectionName).trim().toUpperCase();
        return normalizedResponseSection === normalizedTargetSection;
      }
      
      // Fallback: check if student is enrolled in this section (for legacy data)
      if (r.studentId && r.studentId._id) {
        return section.enrolledStudents.some(s => 
          s._id.toString() === r.studentId._id.toString()
        );
      }
...
exports.getFormResponsesVisualization = async (req, res) => {
  try {
    ...
    const validResponses = responses.filter(r => {
      // Use the stored studentSection field instead of looking up from student profile
      if (!r.studentSection) {
        // Fallback to student profile if studentSection not stored (legacy data)
        if (!r.studentId || !r.studentId.academicInfo) return false;
      ...
      const responseSection = String(r.studentSection).trim().toUpperCase();
      return targetSections.length 
        ? targetSections.some(ts => String(ts).trim().toUpperCase() === responseSection)
        : true;
    });
...
exports.getFacultyFormAnalytics = async (req, res) => {
  try {
    ...
    if (section) {
      ...
      // Normalize the filter section for comparison
      const normalizedFilterSection = String(section).trim().toUpperCase();
      
      // Filter using the stored studentSection field
      filteredResponses = responses.filter(r => {
        // Use the stored studentSection field (more reliable)
        if (r.studentSection) {
          const normalizedStudentSection = String(r.studentSection).trim().toUpperCase();
          return normalizedStudentSection === normalizedFilterSection;
        }
        
        // Fallback: Use populated student academicInfo.section for legacy data
        const studentSection = r.studentId?.academicInfo?.section;
    ...
    form.questions.forEach((question, qIndex) => {
      // Get both possible IDs for matching
      const questionIdStr = question.questionId ? question.questionId.toString() : null;
      const questionSubdocIdStr = question._id ? question._id.toString() : null;
      
      console.log(`  Processing Question ${qIndex + 1}, IDs:`, {
      ...
      let questionResponses = filteredResponses
        .map(r => r.responses.find(resp => {
          ...
          return respIdStr === questionIdStr || respIdStr === questionSubdocIdStr;
        }))
        .filter(Boolean);
      ...
      if (question.type === 'rating') {
        const ratings = questionResponses
          .map(r => {
            const ratingValue = r.rating || r.answer;
            console.log(`      Rating value:`, ratingValue, typeof ratingValue);
            return ratingValue;
          })
