  ...
  try {
    ...
    if (!facultyDepartment) {
      return res.status(400).json({ 
        message: 'Faculty department information is missing. Please contact admin.' 
      });
    }
...
// @desc    Get feedback statistics for the logged-in student
// @route   GET /api/feedback/student-stats
...
exports.getStudentFeedbackStats = async (req, res) => {
  try {
    ...
    // Get all submitted feedback responses by the student
    const submittedResponses = await FeedbackResponse.find({ studentId });

    // Get all active forms - filter by section and department
    const allActiveForms = await FeedbackForm.find({ status: 'active' })
      .populate('facultyId', 'academicInfo.facultyDepartment');

    // Filter forms by student's section and department
    const activeForms = allActiveForms.filter(form => {
      // Check if form targets student's section
      const sectionMatch = studentSection && form.targetSections && 
        form.targetSections.some(ts => 
          String(ts).trim().toUpperCase() === String(studentSection).trim().toUpperCase()
        );
      ...
      const normalizedStudentDept = String(studentDepartment).trim().toUpperCase();
      
      // Check form's department field
      if (form.department) {
        return String(form.department).trim().toUpperCase() === normalizedStudentDept;
      }
      
      // Fallback: check faculty's department
      if (form.facultyId && form.facultyId.academicInfo && form.facultyId.academicInfo.facultyDepartment) {
        return String(form.facultyId.academicInfo.facultyDepartment).trim().toUpperCase() === normalizedStudentDept;
      }
    ...
    const pendingForms = activeForms.filter(form => !submittedFormIds.has(form._id.toString()));

    const submittedCount = submittedResponses.length;
    const impactScore = submittedCount * 10; // Simple calculation
    ...
    console.error('Error in getStudentFeedbackStats:', error);
    res.status(500).json({ message: 'Failed to get student feedback stats' });
  }
};
...
exports.getActiveFeedbackForStudent = async (req, res) => {
  try {
    ...
    console.log('Student Department:', req.user.academicInfo?.department);
    console.log('Student Full Academic Info:', JSON.stringify(req.user.academicInfo, null, 2));

    // Get student's section and department from academicInfo
    let studentSection = req.user.academicInfo?.section;
    let studentDepartment = req.user.academicInfo?.department;
    ...
    if (!studentDepartment) {
      console.log('No department found for student');
      return res.json([]);
    }

    // Normalize the student's section value (trim, uppercase)
    studentSection = String(studentSection).trim().toUpperCase();
    studentDepartment = String(studentDepartment).trim().toUpperCase();

    const currentDate = new Date();
    console.log('Current Date:', currentDate);
    console.log('Filtering by Section:', studentSection, 'and Department:', studentDepartment);
    ...
    // First, get all active forms for the student's section (without department filter)
    const allSectionForms = await FeedbackForm.find({
      status: 'active',
    ...
    console.log('Found forms matching section (before department filter):', allSectionForms.length);
    
    // Filter by department - check both form.department AND facultyId.academicInfo.facultyDepartment
    let forms = allSectionForms.filter(form => {
      // Check if form has department field (new forms)
      if (form.department) {
        const formDept = String(form.department).trim().toUpperCase();
        const match = formDept === studentDepartment;
        console.log(`  Form "${form.title}" has department: ${form.department} - Match: ${match}`);
        return match;
      }
      
      // Fallback: Check faculty's department (for forms without department field)
      if (form.facultyId && form.facultyId.academicInfo && form.facultyId.academicInfo.facultyDepartment) {
        const facultyDept = String(form.facultyId.academicInfo.facultyDepartment).trim().toUpperCase();
        const match = facultyDept === studentDepartment;
        console.log(`  Form "${form.title}" from faculty dept: ${facultyDept} - Match: ${match}`);
        return match;
      }
      
      // If no department info available, exclude for safety
      console.log(`  Form "${form.title}" has NO department info - Excluded`);
      return false;
    });

    console.log('Found forms matching section and department (after filtering):', forms.length);
    forms.forEach(f => {
      console.log(`  Matched form: ${f.title} - Department: ${f.department || f.facultyId?.academicInfo?.facultyDepartment} - targetSections: ${JSON.stringify(f.targetSections)}`);
    });

    // If none found, try fallback with normalized matching
    if (forms.length === 0) {
      ...
      broadForms.forEach(f => {
        console.log(`  Broad form: ${f.title} - Department: ${f.department} - targetSections: ${JSON.stringify(f.targetSections)}`);
      });
      ...
      const broadened = broadForms.filter(f => {
        // First, check department match
        let departmentMatch = false;
        
        if (f.department) {
          const formDept = String(f.department).trim().toUpperCase();
          departmentMatch = formDept === studentDepartment;
        } else if (f.facultyId && f.facultyId.academicInfo && f.facultyId.academicInfo.facultyDepartment) {
          const facultyDept = String(f.facultyId.academicInfo.facultyDepartment).trim().toUpperCase();
          departmentMatch = facultyDept === studentDepartment;
        }
        
        // If department doesn't match, exclude
        if (!departmentMatch) {
          console.log(`  Excluding form "${f.title}" - department mismatch`);
          return false;
        }
        
        // Then check section match
...
exports.submitFeedbackResponse = async (req, res) => {
  try {
    ...
    // Get student's section and department from their academic info
    const studentSection = req.user.academicInfo?.section;
    const studentDepartment = req.user.academicInfo?.department;
    
    if (!studentSection) {
      return res.status(400).json({ 
        message: 'Student section information is missing. Please contact admin.' 
      });
    }
    
    if (!studentDepartment) {
      return res.status(400).json({ 
        message: 'Student department information is missing. Please contact admin.' 
      });
    }

    // Verify student is in the correct department for this form
    if (form.department) {
      const formDepartment = String(form.department).trim().toUpperCase();
      const normalizedStudentDept = String(studentDepartment).trim().toUpperCase();
      
      if (formDepartment !== normalizedStudentDept) {
        return res.status(403).json({ 
          message: `This feedback form is for ${form.department} department only. You are in ${studentDepartment} department.` 
        });
      }
    }
...
exports.getFacultyAnalytics = async (req, res) => {
  try {
    ...
    const responses = await FeedbackResponse.find({
      formId: { $in: formIds }
    }).populate('studentId', 'universityId personalInfo.firstName personalInfo.lastName academicInfo.section academicInfo.department');
    ...
    for (const [section, data] of sectionMap) {
      ...
      // Filter responses by both form ID AND student section AND department
      const sectionResponses = responses.filter(r => {
        // Check if response is for one of this section's forms
        if (!sectionFormIds.includes(r.formId.toString())) {
          return false;
        }
        
        // Check if response is from a student in this specific section
        if (r.studentSection) {
          // Use the stored studentSection field (most reliable)
          const normalizedResponseSection = String(r.studentSection).trim().toUpperCase();
          const normalizedTargetSection = String(section).trim().toUpperCase();
          
          if (normalizedResponseSection !== normalizedTargetSection) {
            return false;
          }
        } else {
          // Fallback: try to check from populated student data (for legacy data)
          if (r.studentId && r.studentId.academicInfo && r.studentId.academicInfo.section) {
            const normalizedStudentSection = String(r.studentId.academicInfo.section).trim().toUpperCase();
            const normalizedTargetSection = String(section).trim().toUpperCase();
            
            if (normalizedStudentSection !== normalizedTargetSection) {
              return false;
            }
          } else {
            // No section info available - exclude
            return false;
          }
        }
        
        // Check department match
        if (r.studentDepartment && data.forms.length > 0 && data.forms[0].department) {
          const normalizedResponseDept = String(r.studentDepartment).trim().toUpperCase();
          const normalizedFormDept = String(data.forms[0].department).trim().toUpperCase();
          return normalizedResponseDept === normalizedFormDept;
        }
        
        // Fallback: check from student profile for legacy data
        if (r.studentId && r.studentId.academicInfo && r.studentId.academicInfo.department) {
          if (data.forms.length > 0 && data.forms[0].department) {
            const normalizedStudentDept = String(r.studentId.academicInfo.department).trim().toUpperCase();
            const normalizedFormDept = String(data.forms[0].department).trim().toUpperCase();
            return normalizedStudentDept === normalizedFormDept;
          }
        }
        
        // If no department info, allow (for backward compatibility)
        return true;
      });
