  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(roleMap[result.data.role] || '/');
      } else {
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box sx={{ mt: 3, p: 2, bgcolor: '#f5f5f5', borderRadius: 2 }}>
              <Typography variant="caption" sx={{ display: 'block', fontWeight: 'bold', mb: 1, color: '#667eea' }}>
                🔐 Admin Login:
              </Typography>
              <Typography variant="caption" sx={{ display: 'block', fontFamily: 'monospace' }}>
                admin@stackhack.edu / admin@123
              </Typography>
              <Typography variant="caption" sx={{ display: 'block', mt: 2, color: '#666', fontStyle: 'italic' }}>
                💡 Use admin to create users via Excel upload
              </Typography>
            </Box>
          </CardContent>
        </Card>
