...
function Protected({ children, allowedRoles }) {
  ...
  // Check both user state and localStorage for consistency
  React.useEffect(() => {
    ...
    if (!token || !storedUser || !user) {
      // This will be handled by the render below
    }
  }, [location, user]);
...
export default function App() {
  ...
  // Only show sidebar when user is logged in AND not on login page
  const showSideNav = user && location.pathname !== '/login';
      ...
      <Box sx={{ 
        ...
        <Routes>
          ...
          <Route path="/student/history" element={<Protected allowedRoles={["student"]}><FeedbackHistory /></Protected>} />
            <Route path="/student/announcements" element={<Protected allowedRoles={["student"]}><AnnouncementsPage /></Protected>} />
          ...
          <Route path="/faculty/responses/:id" element={<Protected allowedRoles={["faculty","department_admin","system_admin"]}><FacultyResponses /></Protected>} />
            <Route path="/faculty/announcements" element={<Protected allowedRoles={["faculty"]}><FacultyAnnouncementsPage /></Protected>} />

          <Route path="/department" element={<Protected allowedRoles={["department_admin"]}><DepartmentDashboard /></Protected>} />
            <Route path="/department/announcements" element={<Protected allowedRoles={["department_admin"]}><FacultyAnnouncementsPage /></Protected>} />
